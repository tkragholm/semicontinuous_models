...
#[must_use]
pub fn fit_tweedie_candidates_input(input: &ModelInput, powers: &[f64]) -> Vec<TweedieCandidate> {
    powers
        .iter()
        .copied()
        .filter_map(|power| {
            fit_tweedie_input(input, power, TweedieOptions::default())
                .ok()
                .map(|(model, _report)| {
                    let pred = model.predict(&input.design_matrix);
                    let metrics =
                        compute_model_fit_metrics(&input.outcome, &pred.mean, Some(power));
                    let ll = tweedie_quasi_log_likelihood(&input.outcome, &pred.mean, power);
                    let information_criteria = compute_information_criteria(
                        ll,
...
pub fn recommend_from_cv(cv: &CrossValidationResult) -> Option<String> {
    ...
    if cv.include_two_part && cv.two_part_metrics.rmse < best_rmse {
    ...
    if cv.include_lognormal
        && let Some(lognormal) = &cv.lognormal_metrics
...
#[allow(
    ...
    reason = "cross-validation keeps fold-local helpers scoped with the orchestration logic"
)]
pub fn cross_validate_models_input(
    ...
    let fold_bounds = (0..options.k_folds)
        .filter_map(|fold| {
    ...
    #[derive(Clone)]
    struct FoldMetrics {
        two_part: ModelFitMetrics,
        lognormal: ModelFitMetrics,
        tweedie: Vec<ModelFitMetrics>,
    }

    let fold_metrics = fold_bounds
        .par_iter()
        .map(
            |&(start, end)| -> Result<FoldMetrics, CrossValidationError> {
                ...
                let mut two_part = zero_metrics();
                let mut lognormal = zero_metrics();
                let mut tweedie = powers.iter().map(|_| zero_metrics()).collect::<Vec<_>>();

                if options.include_two_part {
                    let fitted = fit_two_part_input(&train_input, options.two_part_options)?;
                    let prediction = fitted.0.predict(&test_input.design_matrix);
                    two_part = compute_model_fit_metrics(
                        &test_input.outcome,
                ...
                for (pos, power) in powers.iter().copied().enumerate() {
                    ...
                    let prediction = model.predict(&test_input.design_matrix);
                    tweedie[pos] = compute_model_fit_metrics(
                        &test_input.outcome,
                        &prediction.mean,
                        Some(power),
                    );
                ...
                if options.include_lognormal {
                    let (model, _report) =
                        fit_lognormal_smearing_input(&train_input, options.lognormal_options)?;
                    let prediction = model.predict(&test_input.design_matrix);
                    lognormal =
                        compute_model_fit_metrics(&test_input.outcome, &prediction.mean, None);
                }
    ...
    let folds_used = fold_metrics.len();
    ...
    for fold in fold_metrics {
        let fold = fold?;
        if options.include_two_part {
            add_metrics(&mut two_part_accum, &fold.two_part);
        }
        if options.include_lognormal {
            add_metrics(&mut lognormal_accum, &fold.lognormal);
        }
        for (accum, next) in tweedie_accum.iter_mut().zip(fold.tweedie.iter()) {
            add_metrics(accum, next);
        }
    }
    ...
    let result = CrossValidationResult {
        ...
        lognormal_metrics: options.include_lognormal.then_some(lognormal_accum),
    };
    Ok(result)
}
...
mod tests {
    ...
    fn cross_validate_rejects_invalid_fold_counts() {
        ...
        let low = cross_validate_models_input(
            &input,
            &[1.5],
            CrossValidationOptions::builder().k_folds(1).build(),
        )
        ...
        let high = cross_validate_models_input(
            &input,
            &[1.5],
            CrossValidationOptions::builder().k_folds(n + 1).build(),
        )
    ...
    fn cross_validate_can_skip_two_part_and_lognormal() {
        ...
        let result = cross_validate_models_input(
            &input,
            &[1.2],
            CrossValidationOptions::builder()
                .include_two_part(false)
                .include_lognormal(false)
                .build(),
        )
        .expect("cv should run with only Tweedie enabled");
        assert!(!result.include_two_part);
        assert!(!result.include_lognormal);
        assert!(result.lognormal_metrics.is_none());
        assert_eq!(result.tweedie_candidates.len(), 1);
    }
